    ...
    def vlm_attention(self, image_features, prompts=None, image_counts=None, long_video=False):        
        ...
        for _idx in range(len(prompts)):
            ...
            if image_counts is None:
                ...
                img_feat_prompt = img_feat_prompt.squeeze(0)
                ...
                img_att_prompt = img_att_prompt.squeeze(0)
            ...
            if "qformer" in self.config.bert_type:
                ...
                print("query_tokens size:", query_tokens.size()[:-1])
                print("bert_feat device size:", bert_feat.size())
        ...
        if type(images) is list or images.ndim == 5:
            ...
            if not long_video:
              #  images = [image if len(image.shape) == 4 else image.unsqueeze(0) for image in images]
               images = [image.squeeze(0) for image in images]
            #modify image.shape[0] to 1
            image_counts = [image.shape[1] for image in images]
            #modify the dim from 0 to 1
            concat_images = torch.cat(images, dim=1)
